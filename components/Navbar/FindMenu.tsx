...
const FindMenu = () => {
  ...
  return (
    <div className="w-[65%] relative font-mainR" ref={searchRef}>
      <input
      ...
      />
      {loading ? (
        <div className="absolute top-12 h-[200px] w-full z-50 bg-white flex justify-center items-center shadow-xl">
          <Spin size="large" /> {/* Show loading spinner */}
        </div>
      ) : (
        isDropdownVisible &&
        (results.length > 0 ? (
          <div className="absolute top-12 h-[200px] z-50 text-black bg-white w-full overflow-y-auto rounded-[10px] shadow-xl">
            {results.map((item) => (
              <Link key={item.id} href={`${item.url}/${item.slug}`}>
                <div className="p-2 hover:bg-gray-700 hover:text-white cursor-pointer flex items-center space-x-5">
                  <Image
                    src={item.imageUrl}
                    alt={item.title}
                    width={1000}
          ...
          <div className="absolute top-12 h-[100px] text-[14px] bg-white w-full z-50 flex justify-center items-center shadow-xl">
            Không tìm thấy
